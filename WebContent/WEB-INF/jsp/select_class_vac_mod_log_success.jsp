...
<title>My JSP 'select_class_vac_mod_success.jsp' starting page</title>
...
<body>
	<span style="color: red; float: left;"> <c:out
			value="${select_class_vac_mod_log_null }"></c:out></span>
	<form action="" method="post">
		<table>
			...
			<c:forEach items="${classAftAttendances }" var="attendance"
				varStatus="status">
				<tr>
					...
					<td><fmt:formatDate value="${attendance.at_applyday}"
							pattern="yyyy-MM-dd" /></td>
					<td><a
						href="employee/toSelectPreModAtten.do?at_id=${attendance.at_id }"
						target="menuFrame">查看</a></td>
				</tr>
			</c:forEach>
		</table>
		<br> &nbsp;&nbsp;&nbsp;<a
			href="employee/toSelectClassVacModLog.do" target="menuFrame">&laquo;&laquo;返回继续查询</a>
	</form>
</body>
</html>
