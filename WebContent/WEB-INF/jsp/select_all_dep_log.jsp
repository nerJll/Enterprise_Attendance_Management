...
<title>My JSP 'select_all_dep_log.jsp' starting page</title>
...
<body>
	<span style="color: red; float: left;"> <c:out
			value="${select_all_dep_log_error }"></c:out></span>
	<form action="" method="post">
		...
		<table>
			...
			<c:forEach items="${sessionScope.pagemsg.lists }" var="attendance"
				...
				<tr>
					<td><c:out
							value="${(sessionScope.pagemsg.currPage-1)*8+status.count }" /></td>
			...
			<tr>
				<td><span>第${sessionScope.pagemsg.currPage }/
						...
						每页显示:${sessionScope.pagemsg.pageSize}</span>&nbsp;&nbsp; <span> <c:if
							...
							<a
								href="${projectPath }/employee/toSelectAllDepLog.do?currentPage=1">[首页]</a>&nbsp;&nbsp;
           		<a
								href="${projectPath }/employee/toSelectAllDepLog.do?currentPage=${sessionScope.pagemsg.currPage-1}">[上一页]</a>&nbsp;&nbsp;
       </c:if> <c:if
							test="${sessionScope.pagemsg.currPage != sessionScope.pagemsg.totalPage}">
							<a
								href="${projectPath }/employee/toSelectAllDepLog.do?currentPage=${sessionScope.pagemsg.currPage+1}">[下一页]</a>&nbsp;&nbsp;
           <a
								href="${projectPath }/employee/toSelectAllDepLog.do?currentPage=${sessionScope.pagemsg.totalPage}">[尾页]</a>&nbsp;&nbsp;
       </c:if>
				</span></td>
			</tr>
		</table>
