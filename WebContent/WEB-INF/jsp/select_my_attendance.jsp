...
<title>My JSP 'select_my_attendance.jsp' starting page</title>
...
<body>
	<span style="color: red; float: left;"> <c:out
			value="${select_my_attendance_error }"></c:out></span>
			...
			<c:forEach items="${sessionScope.pagemsg.lists }" var="attendance"
				...
				<tr>
					...
					<td><fmt:formatDate value="${attendance.at_applyday}"
							pattern="yyyy-MM-dd" /></td>
					<td><c:choose>
							<c:when test="${attendance.at_state == '未审核'}">
								<a
									href="employee/toUpdateAttendance.do?at_id=${attendance.at_id }"
									target="menuFrame">修改</a>
							</c:when>
							<c:when test="${attendance.at_state == '不通过'}">
								<a href="employee/toSelectReason.do?at_id=${attendance.at_id }"
									target="menuFrame">查看原因 </a>
							</c:when>
						</c:choose></td>
					<td><c:if test="${attendance.at_state == '未审核'}">
							<a
								href="employee/toDeleteAttendance.do?at_id=${attendance.at_id }"
								target="menuFrame">删除</a>
						</c:if></td>
				</tr>
			</c:forEach>
		</table>
			...
			<tr>
				<td><span>第${sessionScope.pagemsg.currPage }/
						...
						每页显示:${sessionScope.pagemsg.pageSize}</span>&nbsp;&nbsp; <span> <c:if
							...
							<a
								href="${projectPath }/employee/toSelectMyApplyLog.do?currentPage=1">[首页]</a>&nbsp;&nbsp;
           		<a
								href="${projectPath }/employee/toSelectMyApplyLog.do?currentPage=${sessionScope.pagemsg.currPage-1}">[上一页]</a>&nbsp;&nbsp;
       </c:if> <c:if
							test="${sessionScope.pagemsg.currPage != sessionScope.pagemsg.totalPage}">
							<a
								href="${projectPath }/employee/toSelectMyApplyLog.do?currentPage=${sessionScope.pagemsg.currPage+1}">[下一页]</a>&nbsp;&nbsp;
           <a
								href="${projectPath }/employee/toSelectMyApplyLog.do?currentPage=${sessionScope.pagemsg.totalPage}">[尾页]</a>&nbsp;&nbsp;
       </c:if>
				</span></td>
			</tr>
		</table>
